...
public class FieldEncryptInterceptor2 implements Interceptor {
...
//        MappedStatement mappedStatement = (MappedStatement) parameterHandler.getClass().getDeclaredField("mappedStatement").get(parameterHandler);
        ...
        if(SqlCommandType.INSERT == sqlCommandType ||
            ...
            this.execEncrypt(parameter, boundSql);
        }
        return invocation.proceed();
    }

    public static void main(String[] args) throws IllegalAccessException {
        FieldEncryptInterceptor2 interceptor2 = new FieldEncryptInterceptor2();
        ...
        interceptor2.execEncrypt(map, null);

        System.out.println(JSONUtil.toJsonStr(list));
    }

    private void execEncrypt(Object parameter, BoundSql boundSql) throws IllegalAccessException {
        ...
        if(parameter instanceof Map) {
            ...
            map.values().forEach(item -> {
                ...
                this.doGetEncryptVal(item, null, boundSql);
                itemRepeatList.add(item);
            });
        } else {
            this.doGetEncryptVal(parameter, null, boundSql);
//                    this.process(item);
        }
    }

    //只处理bean
    private void process(Object parameter, BoundSql boundSql) {
        if (Objects.isNull(parameter)) {
            return;
        }
        ...
        this.doProcess(parameter, fieldList, boundSql);
    ...
    private void doProcess(Object bean, List<Field> fieldList, BoundSql boundSql) {

        fieldList.stream().forEach(field -> {

            field.setAccessible(true);
            try {
                field.set(bean, this.getEncryptVal(bean, field, boundSql));
    ...
    private Object getEncryptVal(Object parameter, Field field, BoundSql boundSql) throws IllegalAccessException {
        Object fieldBean = field.get(parameter);
        if (fieldBean == null) {
            // 没有值，不需要操作
            return null;
        }
        return this.doGetEncryptVal(fieldBean, field, boundSql);
    }

    private Object doGetEncryptVal(Object fieldBean, Field field, BoundSql boundSql) {
        ...
        if (clazz.isArray()) {
            ...
            return this.encryptNess( (String) fieldBean, field, encrypt, boundSql);
        } else {
            this.process(fieldBean, boundSql);
        }
        return fieldBean;
    }

    private Object encryptNess(String fieldBean, Field field, boolean encrypt, BoundSql boundSql) {
        if(!encrypt) {
            return fieldBean;
        }
        String encryptedValue = fieldBean + "java --苏*锋";
        List<FieldEncryptSnapshotInfo> fieldEncryptSnapshotInfos = (List<FieldEncryptSnapshotInfo>) boundSql.getAdditionalParameter("FieldEncryptInterceptor");
        if(Objects.isNull(fieldEncryptSnapshotInfos)) {
            fieldEncryptSnapshotInfos = new ArrayList<>();
            boundSql.setAdditionalParameter("FieldEncryptInterceptor", fieldEncryptSnapshotInfos);
        }
        FieldEncryptSnapshotInfo info = new FieldEncryptSnapshotInfo();
        info.setField(field);
        info.setOrigin(fieldBean);
        info.setEncrypt(encryptedValue);
        fieldEncryptSnapshotInfos.add(info);
